        ...
        public static object DiffOrDefault(object val1, Type val1Type, object val2, Type val2Type, object defaultValue, bool absolute, bool inclusive, TimeElapsedMethod timeElapsedMethod)
        {
            ...
            try
            {
                if (IsFloat(val1Type))
                {
                    ...
                    if (absolute && diff != null) diff = Math.Abs((double)diff);
                    return (double)diff + ((inclusive) ? (Math.Sign((double)diff)) : (0.0));
                }
                else if (IsNumeric(val1Type))
                {
                    ...
                    if (absolute && diff != null) diff = Math.Abs((Int64)diff);
                    return (Int64)diff + ((inclusive) ? (Math.Sign((Int64)diff)) : (0));
                }
...
#if !DOTNETCORE && ManagementObjects
