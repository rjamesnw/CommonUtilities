...
namespace Common.MVC.Entities
{
    ...
    public interface ITableRow
    {
        ...
        DbEntityValidationResult EntityValidationResult { get; }
    ...
    public class TableRow<TEntity> : ITableRow<TEntity>, IInternalTableRow where TEntity : class, new()
    {
        ...
        public DbEntityValidationResult EntityValidationResult { get; private set; }
        ...
        public Exception SetValue(string propertyName, object value, bool compareWithExistingValue = true)
        {
            ...
            if (propInfo != null)
            {
                try
                {
                    value = Common.Types.ChangeType(value, propInfo.PropertyType);
                }
                catch (Exception ex)
                {
                    ...
                    if (Common.Utilities.IsDateTime(expectedType))
                        msg = "A valid date/time value is expected.";
                    else if (Common.Utilities.IsInt(expectedType))
                        msg = "A valid integer value is expected.";
                    else if (Common.Utilities.IsUInt(expectedType))
                        msg = "A valid positive integer value is expected.";
                    else if (Common.Utilities.IsNumeric(expectedType))
                        msg = "A valid numerical value is expected.";
                    else if (Common.Utilities.IsString(expectedType))
                        msg = "A valid text value is expected.";
                    else if (Common.Utilities.IsBoolean(expectedType))
        ...
        public bool Load(IDictionary<string, string[]> requestProperties, Int64 rowIndex)
        {
            ...
            Deleted = Common.Utilities.ND(reqProps.Value(rowPrefix + "Deleted")?.FirstOrDefault(), false);
            New = Common.Utilities.ND(reqProps.Value(rowPrefix + "New")?.FirstOrDefault(), false);
            ...
            {
                var entitySet = Table.Context.Set(EntityType);
                ...
                foreach (var key in Table.KeyNames)
                {
                    ...
                    if (propInfo != null)
                    {
                        try
                        {
                            postedValue = Common.Types.ChangeType(reqProps.Value(keyPostID)?.FirstOrDefault(), propInfo.PropertyType);
                            ...
                            if (postedValue is string && string.IsNullOrWhiteSpace((string)postedValue)
                                ...
                                || postedValue != null && Common.Utilities.IsNumeric(postedValue.GetType()) && postedValue.ToString() != "0"
        ...
        public string GetDisplayText(ITableColumn<object> col)
        {
            ...
            if (value != null)
                ...
                else
                {
                    ...
                    else if (Common.Utilities.IsFloat(value.GetType()))
                    {
        ...
        /// </summary>
        public bool HasValidEntityKey { get { return (from v in GetKeyValues() where Common.Utilities.ND(v, (Int64)0) == 0 select v).Count() == 0; } }
        ...
        public ValidationResults Commit(DbContext context = null, bool save = true)
        {
            ...
            if (ValidationResult == ValidationResults.Valid)
            {
                if (save && context.ChangeTracker.HasChanges())
                    ...
                    {
                        _CommitError = e.ExceptionOf<System.Data.SqlClient.SqlException>().FirstOrDefault() ?? e;
                    }
    ...
    public class TableColumn<TEntity> : ITableColumn<TEntity>, ITableColumn, IInternalTableColumn where TEntity : class, new()
    {
        ...
        public static TableColumn<TEntity> FromMetadataProperty(ITable<TEntity> table, EdmProperty prop)
        {
            ...
            int? maxLen = maxLenAttr != null ? maxLenAttr.Length : prop.MaxLength;
            bool isKey = keyAttr != null || ((EntityType)prop.DeclaringType).KeyMembers.Contains(prop); // (Note: Casting the declaring type and checking the key members is how the inaccessible internal '{EdmProperty}.IsKeyMember' property works)
            string requiredMsg = requiredAttr != null ? requiredAttr.ErrorMessage : null;
            bool? nullable = prop.Nullable;
            bool hidden = hiddenAttr != null;
    ...
    public interface ITable
    {
        ...
        bool Load(HttpRequestBase request, string tableID);
        ValidationResults ApplyChanges(DbContext context);
        ValidationResults ApplyChanges<TEntities>()
           where TEntities : DbContext, new();
        ValidationResults Commit(DbContext context = null, bool save = true);
    ...
    public class Table<TEntity> : ITable<TEntity>, ITable where TEntity : class, new()
    {
        ...
        public DbContext Context
        {
            ...
            private set
            {
                ...
                _SSpaceItems = (from et in _ObjectContext.MetadataWorkspace.GetItemCollection(DataSpace.SSpace) where et.BuiltInTypeKind == BuiltInTypeKind.EntityType select et).ToArray();
                _CSpaceItems = (from et in _ObjectContext.MetadataWorkspace.GetItemCollection(DataSpace.CSpace) where et.BuiltInTypeKind == BuiltInTypeKind.EntityType select et).ToArray();
                _CSSpaceItems = (from et in _ObjectContext.MetadataWorkspace.GetItemCollection(DataSpace.CSSpace) where et.BuiltInTypeKind == BuiltInTypeKind.MetadataItem select et).ToArray();
            }
        }
        DbContext _DBContext;
        ObjectContext _ObjectContext;
        GlobalItem[] _SSpaceItems; // (Storage) Database storage details.
        GlobalItem[] _CSpaceItems; // (Conceptual) CLR type details.
        GlobalItem[] _CSSpaceItems; // (Storage<->Conceptual) Mapping details.
        ...
        internal MetadataTypeAttribute _MetadataTypeAttribute = new MetadataTypeAttribute(typeof(DBNull));
        ...
        public Type GetCustomMetadataType(Type entityType) { return _MetadataTypeAttribute?.MetadataClassType; }
        public IEnumerable<TAttribute> GetCustomMetadataPropertyAttributes<TAttribute>(string propertyName, Type entityType = null) where TAttribute : Attribute
        {
            ...
            // (the "_MetadataTypeAttribute.MetadataClassType" holds the meta-data type that contains the model data annotations)

            if (_MetadataTypeAttribute != null && _MetadataTypeAttribute.MetadataClassType == typeof(DBNull))
                _MetadataTypeAttribute = (MetadataTypeAttribute)entityType.GetCustomAttributes(typeof(MetadataTypeAttribute), true).FirstOrDefault();

            if (!string.IsNullOrWhiteSpace(propertyName))
            {
                var metaType = _MetadataTypeAttribute != null ? _MetadataTypeAttribute.MetadataClassType : entityType;
        ...
        /// </summary>
        public Table(string id, HttpRequestBase request)
            : this(request["TableTitle"], request["KeyName"])
        {
        ...
        public bool Load(HttpRequestBase request, string tableID = null)
        {
        ...
        public ITable<TEntity> BuildColumns(bool rebuild = false)
        {
            if (_TableColumns == null || _TableColumns.Count == 0 || rebuild)
            {
                ...
                else
                {
                    ...
                    // (it is possible that TEntity is a common BASE type, so each row needs to be checked just in case)
        ...
        where TEntity : class, new()
    {
        ...
        /// </summary>
        public TableSet(HttpRequestBase request)
        {
            Load(request);
        }
        ...
        /// </summary>
        public TableSet<TEntity> Load(HttpRequestBase request)
        {
            ClearValidations();
    ...
    /// </summary>
    /// <typeparam name="TEntity"></typeparam>
    public class TableSetBinder<TEntity> : IModelBinder
        where TEntity : class, new()
    {
        object IModelBinder.BindModel(ControllerContext controllerContext, ModelBindingContext bindingContext)
        {
            return new TableSet<TEntity>(controllerContext.HttpContext.Request);
        }
    }
}