...
using CPP.Framework;
using Shared.CollectionsAndLists;
...
namespace Shared.Silverlight.Attachables
{
    ...
    public class StyleLoader : DependencyObject
    {
        ...
        /// derrived from 'Control' - in which, normally, this is not supported.</remarks>
        /// </summary>
        /// <param name="filename">File name, such as "generic.xaml".</param>
        /// <param name="path">A sub-folder the rsource is located in, if any.</param>
        ...
        public static object LoadResource(string filename, string path = "", string assembly = "")
        {
            try
            {
                object resource = Utilities.LoadXAML<object>(filename, path, assembly, BEGIN_RESOURCE_TAG, END_RESOURCE_TAG);
        ...
        public static void OnEventsChanged(DependencyObject d, DependencyPropertyChangedEventArgs e)
        {
            ...
            if (!Utilities.InDesignMode && e.NewValue.ToString() != "" && Application.Current.GetType() != typeof(Application))
            {
